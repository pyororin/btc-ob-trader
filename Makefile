...
migrate: ## Run database migrations
	...
	sudo -E docker compose exec -T -e POSTGRES_USER=$(DB_USER) -e POSTGRES_DB=$(DB_NAME) timescaledb sh -c 'if ls /docker-entrypoint-initdb.d/migrations/*.sql >/dev/null 2>&1; then for f in /docker-entrypoint-initdb.d/02_migrations/*.sql; do psql -v ON_ERROR_STOP=1 --username "$$POSTGRES_USER" --dbname "$$POSTGRES_DB" -f "$$f"; done; else echo "No migration files found."; fi'
