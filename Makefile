...
report: ## Generate and display the PnL report.
	...
	sudo -E docker compose exec $(shell awk -F= '!/^\s*#/ && NF > 0 {print "-e " $$1"="$2}' .env | tr -d '"') report-generator go build -o build/report cmd/report/main.go
	@echo "Running PnL report..."
	sudo -E docker compose exec $(shell awk -F= '!/^\s*#/ && NF > 0 {print "-e " $$1"="$2}' .env | tr -d '"') report-generator ./build/report
