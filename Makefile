...
report: ## Generate and display the PnL report.
	...
	sudo -E docker compose exec $(shell awk 'NF > 0 && !/^\s*#/ {print "-e " $$0}' .env | xargs) report-generator go build -o build/report cmd/report/main.go
	@echo "Running PnL report..."
	sudo -E docker compose exec $(shell awk 'NF > 0 && !/^\s*#/ {print "-e " $$0}' .env | xargs) report-generator ./build/report
