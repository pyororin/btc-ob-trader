...
- `make monitor`: **取引Botを除く**、モニタリング関連のサービス（DB, Grafana）のみを起動します。本番トレードのデータを分析する際に使用します。
- `make replay`: 過去のデータを用いてバックテストを実行します。このコマンドはDBやGrafanaを起動しません。
- `make down`: すべてのサービスを停止します。
- `make help`: 利用可能なすべてのコマンドとその説明を表示します。

### 一般的な開発フロー

1.  **本番トレードのデータ分析のみを行う場合**:
    ```bash
...
2.  **バックテストを実行する場合**:
    ...
    make replay
    ```
    完了後、プロジェクトルートディレクトリに生成される `replay_result.csv` を確認します。
...
### Q4: `make replay` が動作しない。

**A4:**
- `Makefile` を確認し、`replay` 機能の現在の実装ステータスを確認してください。
- `replay` 機能が実装されている場合、必要なデータファイルや設定 (`config-replay.yaml` など) が正しく配置されているか確認してください。
- 出力されるログにエラーメッセージがないか確認してください。
...
リプレイモードは、過去の市場データ（主に取引履歴）をCSVファイルから読み込み、あたかもリアルタイムでデータを受信しているかのようにBotを動作させる機能です。これにより、実際の資金を使わずに、特定の期間の市場に対する戦略のパフォーマンスを検証・デバッグできます。
...
1.  **リプレイ用データの準備**:
    - リプレイに使用する過去データが `config/config-replay.yaml` の `replay.csv_path` で指定されたパスに存在する必要があります。
    - データ形式は、リプレイ機能が読み込める形式（例: `id,order_id,created_at,amount,rate,side`）に準拠している必要があります。

2.  **リプレイ用設定ファイルの確認**:
    - `config/config-replay.yaml` ファイルがリプレイの挙動を制御します。
    - この設定ファイルで、使用する戦略パラメータや、結果を出力するCSVファイルのパス (`replay.output_csv_path`) を指定します。
...
3.  **リプレイコマンドを実行します。**
    ...
    make replay
    ```
    このコマンドは、`bot` サービスをリプレイモードで起動します。データベースやGrafanaは起動しません。
...
リプレイが完了した後、結果は `config/config-replay.yaml` の `replay.output_csv_path` で指定されたファイルに出力されます（デフォルトは `replay_result.csv`）。

1.  **出力されたCSVファイルを開きます。**
    VS Code、Excel、または任意のテキストエディタでCSVファイルを開きます。

2.  **結果を分析します。**
    CSVには、リプレイ中の仮想的な取引履歴や損益の推移などが記録されています。
    これらのデータをもとに、戦略のパフォーマンスを評価します。

## 8. パフォーマンス可視化 (Grafana)

プロジェクトには、**本番トレード**のパフォーマンス（特に損益）を視覚的に監視するための Grafana ダッシュボードが含まれています。リプレイ結果の可視化は現在サポートされていません。
...
2.  **データソース**:
    ダッシュボードは `TimescaleDB (Production)` データソースに接続されており、`make up` で稼働している本番Botのデータを表示します。

3.  **表示パネルの説明**:
    - **Cumulative PnL Over Time**: 累積損益の推移を時系列グラフで表示します。
...
4.  **時間範囲の調整**:
    ダッシュボードの右上にある時間範囲セレクターで、表示するデータの期間を自由に変更できます。
