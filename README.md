...
`config/config.yaml` が基本的な設定ファイルです。取引ペアや戦略パラメータを調整できます。各項目の詳細はファイル内のコメントを参照してください。
...
**設定:**
バックテストの期間は `config/config.yaml` 内の `replay` セクションで設定します。
...
シミュレーションには `config/config.yaml` の戦略パラメータが使用されます。このファイルを編集して、検証したい値を設定してください。
